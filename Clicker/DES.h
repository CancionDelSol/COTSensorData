...
void convertToBinary(int n)
{
    ...
    for (int i = 7; i >= 0; i--)
    {
        ...
        if (k == 0) {
            // Don't print to file, in the original
            //fprintf(out, "0");
        }
        else {
            // Don't print to file, in the original
...
void Encryption(long int plain[])
{
    ...
    for (int i = 0; i < 64; i++) {
        // Don't write to file, in the original
...
void convertToBits(int ch[], int index)
{
    ...
    // fprintf(out, "%c", value); // TODO : find out what file this was supposed to be
    bits[index] = value;
}
...
{
    // Don't open file, in the original
...
// Grab characters from key_source and
//  convert to 64 byte key
const char key_source[BUFFER_SIZE] = { 0 }; // TODO : Create a key
void create16Keys()
{
    // Do not open file for key, in the original
    //FILE* pt = fopen("key.txt", "rb");
    unsigned int key[BITTEDNESS];
    int i = 0, ch;
 
    while (/*!feof(pt)*/ i < BUFFER_SIZE)
    {
        // Don't grab character from file, in the original
        ch = key_source[i];// getc(pt);
        key[i++] = ch - 48;
    }
